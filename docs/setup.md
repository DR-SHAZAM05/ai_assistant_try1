...
- Set `TELEGRAM_BOT_TOKEN`, a random 32-256 character URL-safe `TELEGRAM_WEBHOOK_SECRET` (`A-Z`, `a-z`, `0-9`, `_`, `-`), a comma-separated `TELEGRAM_ALLOWED_USER_IDS` allow-list, and a public HTTPS `TELEGRAM_WEBHOOK_URL`. All four are mandatory when a Telegram bot is enabled in production; placeholder or weak values prevent startup. When all values are valid, application startup registers the Bot API commands and webhook automatically.
- After rotating a bot token or changing any Telegram setting in `.env`, recreate the application container so Docker reloads its environment: `docker compose up -d --build --force-recreate --no-deps app`. The public tunnel named by `TELEGRAM_WEBHOOK_URL` must be running and forwarding to local port `8000`; for an ngrok reserved URL, run `ngrok http --url=https://<your-reserved-ngrok-domain> 8000` in a dedicated PowerShell window. Verify the result with `getWebhookInfo` or by sending `/start` to the bot.
